...
class PDF  extends PDF_Rotate
{
   ...
   ///Logo
    ...
    $this->SetX(15);
    $sta=mysqli_query($conexion,"SELECT tienda_compras_productos.`status` FROM
tienda_compras_productos WHERE nro_orden='".$_GET['cdg']."'");
    ...
    if($dat['status']!='1'){
      ...
      $this->SetFont('Arial','B',90);
      $this->SetTextColor(222,222,222);
       $this->RotatedText(40,190,'NO PAGADO',40);
     }
...
FROM
tienda_compras where nro_orden='".$_GET['cdg']."'");
...
$rows=mysqli_fetch_array($cli);
    ...
    $this->Cell(110,5,'ORDEN #'.$_GET['cdg'],0,0,'C');//orden de compra
    $this->SetFont('ARIAL','',10);
    $this->Ln();
     $this->SetFont('ARIAL','B',10);
   ...
   INNER JOIN tienda_productos ON tienda_compras_productos.id_producto = tienda_productos.id_producto
    ...
    $this->Cell(20,5,$v_money['signo'].':'.number_format($fr['costo'],2,',','.'),LRTB,L,'L',false);
    $this->Cell(20,5,$v_money['signo'].':'.number_format($fr['cantidad']*$fr['costo'],2,',','.'),RLTB,L,'L',false);
    $this->Ln();
...
$this->Cell(20,5,$v_money['signo'].':'.number_format($sum,2,',','.'),LRTB,L,'L',false);
...
$pdf->TablaSimple($header);
$pdf->Output('Comprobante_de_compra.pdf','D');
?> 
